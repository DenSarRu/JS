 ...
 * @param {string} category сюда должно прилетать значение атрибута data-type у
 * кнопки, по которой кликнули.
 */
function showCategory(category) {
    let showEl = '';
    products[category].forEach(function (product) {
        showEl += getProductMarkup(product);
    });
    divProductEl.insertAdjacentHTML('afterbegin', showEl);
};
...
let siteButtons = document.querySelectorAll('button');
let divProductEl = document.querySelector('.products');
siteButtons.forEach(function (buttonEl) {
